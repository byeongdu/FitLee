...
FitLee_helpstr = {'Schultz polydisperse cylinder length form factor (axial direction).' ,...
'Data should be plotted as $q$ vs $I(q) \cdot q$.',...
'',...
'$I(q) = I_0 \cdot \dfrac{\left<H^2 \cdot \mathrm{sinc}^2(q H / 2)\right>}{q - qz_0} + I_b$',...
...
'Average is implemented as $\sum n(H_i) \cdot H_i^2 \cdot \mathrm{sinc}^2(q H_i/2)$ on a 55-point',...
'non-uniform grid spanning $[(H/10)^{1/1.5}, (H + 9 H \cdot sigH)^{1/1.5}]^{1.5}$.',...
'',...
'$Parameters$',...
'$\quad  I\_0$ : scale factor',...
'$\quad  qz0$ : $q$-offset (origin shift) (A$^{-1}$)',...
'$\quad  H$ : Schultz peak cylinder length (A)',...
'$\quad  sigH$ : Schultz relative width (FWHM = $H \cdot sigH$)',...
'',...
