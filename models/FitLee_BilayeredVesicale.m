...
FitLee_helpstr = {'Bilayered vesicle: head-tail-head trilayer with polydisperse core radius.' ,...
'',...
'$I(q) = I_0 \cdot I_{cs}(q) + I_1 \cdot P_1(q) \cdot S(q; D, v_f) + I_b$',...
'$\qquad    I_{cs}(q)$ from internal coreshell helper (polydisperse $R_c$, fixed head $t_h$ and tail $t_t$)',...
'$\qquad    P_1(q) = \left<V_1\right>^2 \cdot $ SchultzsphereFun normalized form factor',...
'$\qquad    I_b = poly1 \cdot q^{poly2} + poly3 \cdot q + poly4$',...
'',...
'Note: the second sphere population uses $I_1$ as a pre-scale (legacy); $I_0$ likewise.',...
'For absolute-unit fitting consider FitLee_schultzsphere2 / FitLee_coreshell_schultzsphere.',...
'$fsig$ = fractional Schultz width (FWHM = $r \cdot fsig$).',...
'',...
'$Parameters$',...
'$\quad  I\_0$ : scale factor for the vesicle population (legacy units)',...
'$\quad  R\_c, fsigCore$ : core radius peak (A) and Schultz relative width',...
'$\quad  th, t_{h,fsig}$ : head-group thickness (A) and its relative width',...
'$\quad  tt$ : tail thickness (A)',...
'$\quad  head\_rho, tail\_rho, solvent\_rho$ : electron densities (interpreted as scaled SLD)',...
'$\quad  I\_1$ : scale for second sphere population (set 0 to disable)',...
'$\quad  r1, fsig\_1$ : radius peak (A) and relative width for second sphere',...
'$\quad  D, vf$ : hard-sphere $S(q)$ parameters (applied to second sphere only)',...
'',...
