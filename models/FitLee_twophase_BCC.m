function [out, report] = FitLee_twophase_BCC(varargin)
...
FitLee_helpstr = {'Two-phase 3D BCC lattice diffraction (e.g. diblock copolymer).' ,...
'',...
'$I(q) = I_0 \cdot $ bcc_twophase$(q, [d, v_f], \Delta\rho, domainsize, microstrain, DW) + I_b$',...
'$\qquad    I_b = poly1 \cdot q + poly2$',...
'',...
'BCC unit cell: 2 spheres of phase A in a cubic lattice of edge $d$.',...
'Sphere radius is back-computed as $R = (v_f \cdot d^3 / 2 / (4\pi/3))^{1/3}$.',...
'',...
'$Parameters$',...
'$\quad  I0$ : overall scale factor',...
'$\quad  d$ : lattice constant (A)',...
'$\quad  vf$ : volume fraction of phase A',...
'$\quad  drho$ : electron-density contrast between phase A and the other',...
'$\quad  domainsize$ : crystal domain size (A); sets the Lorentzian peak width',...
...
Iq = bcc_twophase(q, [p.d, p.vf], p.drho,p.domainsize,p.microstrain, p.DW);
