function [out, report] = FitLee_poly_fractalparticle3(varargin)
FitLee_helpstr = {'Polydisperse mass-fractal particles. ' ,...
'I(q) = I0 \cdot (P_0(q) \cdot S_0(q)+N_{ratio1}P_0(q) \cdot S_1(q)+P_2(q)\cdot S_2(q)) + I_b(q)',...
'    P_2(q) = guinierporodmodel(q, I0_2, s_2, Rg_2, P_2)',...
'    I_b(q) = poly1 \cdot q^{poly2} + poly3 \cdot q + poly4',...
'Parameters',...
'\bf P(q): \rm Form factor of PP',...
'  \rm I0 : Scale factor I0',...
'  avgR : Schultz size distribution, radius peak (A)',...
'  z: Schultz size distribution, FWHM (A)',...
'  D_f : mass fractal dimension. Set D_f<3 or 4.',...
'',...
'\bf S(q) = P_{cluster}(q) + S_{local}(q):',...
' \bf P_{cluster}(q) \rm = guinierporodmodel(q, I0, d, Rg, s)',...
'   \rm I0 : Guinier I0, meaning the number of primary particles forming a cluster',...
'   Rg : Rg of the cluster',...
'   P : Porod exponent of the cluster',...
'   s : dimensionality of the cluster',... 
' \bf S_{local}(q)',...
'  \rm Rh : Hydrodynamic radius (A) (hard sphere potential S(q))',...
...
Pq1 = V2*Pq1;
[Pq0, V0] = SchultzsphereFun(q, p.r0, p.sig0);
Pq0 = V0*Pq0;
