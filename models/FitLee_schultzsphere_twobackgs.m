...
FitLee_helpstr = {'Schultz polydisperse sphere with two user backgrounds, absolute unit $(cm^{-1}). $' ,...
'',...
'$I(q) = I_{pw} + (fn0 \cdot r_e^2 / 10^{-8}) \cdot \delta\rho_0^2 \cdot P_0(q) \cdot S_a(q) + P_2(q) + I_b$',...
'$\qquad    I_{pw} = powI / q^{PorodExp}$',...
'$\qquad    S_a(q) = S_{HS}(q; D, v_f) + P_1(q)$',...
'$\qquad    P_0(q) = $ SchultzsphereFun$(q, r_0, sig_0)$',...
'$\qquad    P_1(q) = I_{0,1} \cdot $ guinierporodmodel$(q, 1, P_1, Rg_1, s_1)$',...
'$\qquad    P_2(q) = I_{0,2} \cdot $ guinierporodmodel$(q, 1, P_2, Rg_2, s_2)$',...
'$\qquad    I_b = poly1 \cdot q^{poly2} + poly3 \cdot q + poly4$',...
'$\qquad\qquad + SF_{userBG1} \cdot UBG_1 + SF_{userBG2} \cdot UBG_2$',...
'',...
'Note: $P_1$ is added into the structure factor (not as an independent term);',...
'$P_2$ is the only fully independent cluster/Guinier-Porod component.',...
'',...
'$Parameters$',...
'$\quad  fn0$ : volume fraction of particle 0 (dimensionless)',...
'$\quad  delta\_rho0$ : electron density contrast for particle 0 $(A^{-3})$',...
'$\quad  r0, sig0$ : Schultz radius peak (A) and FWHM (A)',...
'$\quad  I0\_1, P\_1, Rg\_1, s\_1$ : Guinier-Porod component folded into $S(q)$',...
'$\quad  I0\_2, P2, Rg2, s\_2$ : Guinier-Porod cluster component added on top',...
'$\quad  D$ : interparticle distance (A) for hard-sphere $S_{HS}(q)$',...
'$\quad  vf$ : volume fraction in hard-sphere $S_{HS}(q)$',...
'$\quad  powI, PorodExp$ : low-q power-law term $I_{pw} = powI / q^{PorodExp}$',...
'$\quad  SF_{userBG1}, SF_{userBG2}$ : scale factors for user backgrounds 1 and 2',...
'',...
